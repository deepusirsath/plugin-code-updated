    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.tabs.sendMessage(
        ...
        (response) => {
          ...
          async function fetchCombinedData() {
            try {
              ...
              if (dispute_count >= 0) {
                sendResponse({
                  status:
                    emailStatus === "Dispute" ? "Dispute" : emailStatusData,
          ...
          async function checkDisputeStatus(messageId, email) {
...
async function sendEmlToServer(messageId, blob = null, client, user_email) {
  try {
    ...
    const currnetCode = serverData.code;
    console.log("Server Response currnetCode:", currnetCode);
    // Check if the server response indicates that the data is not available
    if (isErrorResponse(currnetCode)) {
      console.log(
        "The data is not available on the server and server is giving 4 || 404"
      );
      handleEmailNotFoundResponse(messageId, client, activeTabId);
      return false;
    }

    // Handle successful response
    console.log(
      "Response received from the server is passed and handling response"
    );
...
async function checkPendingResponseStatus(messageId, email, client) {
  ...
  try {
    ...
    console.log("Data.code for Pending request ", currnetCode);
    if (isErrorResponse(currnetCode)) {
      console.log(
        "The data is not available on the server and server is giving 4 || 404"
      );
...
function isErrorResponse(code) {
  return code === 404 || code === 4;
}
function handleEmailNotFoundResponse(messageId, client, tabId) {
  chrome.tabs.sendMessage(tabId, {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "sendGmailData") {
    ...
    emlExtractionGmail(eml_Url, currentMessageId, emailId);
  }
});
...
async function checkAdminComment(messageId, email, sendResponse) {
  ...
  try {
    ...
    const data = await response.json();
    console.log(data?.data[0]?.admin_comment, "data?.data[0]?.admin_comment");
    sendResponse({
      adminRemark: data?.data[0]?.admin_comment,
    });
  } catch (err) {
    console.error(err);
  }
